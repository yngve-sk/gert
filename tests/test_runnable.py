...
async def test_end_to_end_local_run(clean_storage: None) -> None:
    ...
    workdir_base = Path("./gert_storage/workdirs") / execution_id
    assert workdir_base.exists()
    # Check realization 0 and 1 workdirs
    for i in range(2):
        realization_dir = workdir_base / f"realization-{i}" / "iteration-0"
        ...
        with (realization_dir / "response.json").open() as f:
            ...
            client.post(f"/storage/{experiment_id}/ingest", json=payload)
