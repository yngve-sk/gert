...
class TestESEdgeCases:
    ...
    def test_missing_responses_handled_correctly(self, es: ESUpdate) -> None:
        ...
        # Realization 1 and 3 are MISSING
        responses = pl.DataFrame(
            {
                "realization": [0, 2],
                "well_id": ["W1", "W1"],
        ...
        # Missing realizations (1 and 3) should retain their prior values exactly
        assert posterior.filter(pl.col("realization") == 1)["PARAM1"].item() == 2.0
        assert posterior.filter(pl.col("realization") == 3)["PARAM1"].item() == 4.0
