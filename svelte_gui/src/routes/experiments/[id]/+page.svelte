				...
				<table class="w-full text-left text-sm whitespace-nowrap">
					<thead class="bg-surface-800 border-b border-surface-700 text-xs uppercase tracking-wider text-surface-400">
						<tr>
							...
							<th class="px-4 py-3 font-semibold text-right">Iteration</th>
							<th class="px-4 py-3 font-semibold text-right">Completed</th>
							<th class="px-4 py-3 font-semibold text-right">Failed</th>
							...
							<tr class="hover:bg-surface-700/50 transition-colors group">
								<td class="px-4 py-3">
									<div class="flex flex-col">
										<span class="font-mono text-primary-400 group-hover:text-primary-300">{exec.execution_id.split('_').pop() || exec.execution_id}</span>
										<span class="text-[10px] text-surface-500 font-mono" title={exec.execution_id}>{exec.execution_id.substring(0, 8)}...</span>
									</div>
								</td>
								<td class="px-4 py-3">
								...
								<td class="px-4 py-3 text-center">
									...
									<a
										...
										class="btn bg-surface-700 hover:bg-surface-600 text-surface-100 text-xs px-3 py-1 rounded transition-colors"
									>
										Inspect
									</a>
								</td>
