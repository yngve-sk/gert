...
* **Avoid Intermediate State Transformation:** When consuming data from external APIs or internal services, prefer storing the resulting domain models (e.g., Pydantic `BaseModel` instances) directly in the component's state rather than extracting specific fields into intermediate dictionaries or primitive variables. This reduces indirection, maintains the integrity of the data model, and ensures that all relevant context remains available for future use or debugging without requiring additional transformations.
